function thresholdedData = f_thresholdRMData(rm,h)
...
%         sometimes there are a lot of additional fields I want to keep so
%         let's just set al the fields here and then replace them
        thresholdedData{goodsubs} = rm{r};
...
%         want to keep the original size, because we might want to care how
%         big the roi was while still using a subset of the data to do
%      another analysis
        thresholdedData{goodsubs}.origindices = rm{r}.indices;
