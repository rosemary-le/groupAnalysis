...
%this one shows the prf amplitudes on the place selective vs  ventral anat rois
% last run 3/20/14
...
%'facesVSplaces01'; faces vs place contrast
   % 'anat'    anatomy
    ...
    % 'ph'    is polar angle
    % 'map'   is eccentricity
%    'ipsi cov map' means loading the ipsi coverage parameter into the map
%    field
...
% input.map = 'prf size';
% input.map = 'polar angle';
input.map = 'eccentricity';

% sometimes you want to rotate the phase
% input.phoffsets = [270*ones(1,length(input.sessions)) 0];
...
% input.savepath =  '/Volumes/biac4-kgs/Projects/retinotopy/adult_ecc_karen/Analyses/MeshImages/prf_ipsiCoverage40114/'

input.savepath = ...
    '/biac4/kgs/biac3/kgs4/projects/retinotopy/adult_ecc_karen/Analyses/MeshImages/adult15degpRFecc05/';
...
% input.roi = {'both_VOanatXcospvf_nw','both_PHCanatXcospvf_nw','both_antHIPPanatXcospvf_nw'};

input.roicolors = [0 0 0];
% [1 0 0; 0 1 0; 1 1 0];%ventral cos anat x pVf rois
